...
async function seedBikes(parks) {
    if (parks.length === 0) {
    ...
    for (const park of parks) {
        const shuffledImages = faker_1.faker.helpers.shuffle(bikeImages);
        for (let i = 0; i < 15; i++) {
            totalBikes++;
...
function createRandomBike(park, status, bikeNumber, image) {
    return {
        model: `Bike-${bikeNumber}-${park.id}`,
        status,
        lock: faker_1.faker.datatype.boolean(),
        location: faker_1.faker.location.streetAddress(),
        price: faker_1.faker.number.int({ min: 2, max: 5 }),
        park_id: park.id,
        image,
    };
}
//# sourceMappingURL=bikes.seed.js.map